...
struct KF {
    ...
    float D_MAX;        /* Number of neighbours */
...
/* ---------------------------------------------------------------------------------- */
int arquitec(char *filename,struct KF *kf,struct DATOS *datos, float d){
  FILE *b;
  char filepat[100];
  int i,j;
  /*bandera de error*/
...
void bubbleSort(int orden[], float distance[],  int n, float **datos, float* input, int d){ 
  int i, j; 
  for (i = 0; i < n; i++) for (j = 0; j < n-i; j++) if (distance[j] > distance[j+1])
  swapf(&distance[j], &distance[j+1]); 
  swapi(&orden[j], &orden[j+1]); 
}   
//---------------------------------------------------------------------
...
// Merge Function
void merge(int orden[], float distancia[], int l, int m, int r, float **datos, float* input, int d){
  int i, j, k;
  int n1 = m - l + 1;
  int n2 = r - m;
  // Create temp arrays
  ...
  for (i = 0; i < n1; i++) L[i] = distancia[l + i];
  for (j = 0; j < n2; j++) R[j] = distancia[m + 1+ j];
  // Merge the temp arrays
  i = 0;
  j = 0;
  k = l;
  while (i < n1 && j < n2){
    if (L[i] <= R[j]){//(dist(d,datos[L[i]],input) <= dist(d,datos[R[j]],input)){
      distancia[k] = L[i];
      orden[k] = l + i;
      i++;
    }
...
void mergeSort(int orden[],float distancia[] ,int l, int r, float **datos, float* input, int d){
  if (l < r){
    // Finding mid element
    int m = l+(r-l)/2;
    // Recursively sorting both the halves
    mergeSort(orden, distancia, l, m, datos, input, d);
    mergeSort(orden, distancia, m+1, r, datos, input, d);
  
    // Merge the array
    merge(orden, distancia, l, m, r, datos, input, d);
  }
}
...
void sorttest(struct KF *kf,struct DATOS *datos){
  ...
  int orden[datos->PR];
  float distancia[datos->PR];

  //inicializacion
  for(int k=0;k<datos->PR;k++){
    //int s=datos->seq[k];
    orden[k]=datos->seq[k];
    distancia[k]= dist(datos->N_IN,test,datos->data[orden[k]]);
  }
  
  //mergeSort(orden,distancia,0,datos->PR-1,datos->data,test,kf->N_IN);
  bubbleSort(orden, distancia, datos->PR-1,datos->data, test, kf->N_IN);
...
int output(struct KF *kf,struct DATOS *datos,float *input){
  ...
  for(k=0;k<datos->PR;k++){
    //int s=datos->seq[k];
    orden[k]=datos->seq[k];
    distancia[k]= dist(datos->N_IN,input,datos->data[orden[k]]);
  ...
  bubbleSort(orden, distancia, datos->PR-1,datos->data, input, kf->N_IN);
  //mergeSort(orden,distancia,0,datos->PR-1,datos->data,input,kf->N_IN);

  
  for(i=0;(distancia[i]<kf->D_MAX) && (i<datos->PR);i++){
...
int main(int argc, char **argv){
  ...
  error=arquitec(argv[1],&kf,&datos,d);
  if(error){

...
struct MLP {
    ...
    float u;            /* Momentum */
    float GAMMA;        /* Weight decay gamma*/
  ...
  for (iter=1; iter<=nn->ITER; iter++) {
    ...
    for(nu1=0;nu1<datos->PR;nu1++) {
	  ...
	  for( j=1 ;j<=nn->N2; j++){
		 suma = 0.0;
		 for( i=1 ;i<=nn->N3; i++)
			suma += nn->grad3[i] * nn->w2[i][j];//aca hacer la suma?y en grad3?
    ...
    //Esta mal, creo que tengo que editar la linea 606, sumandole 2*gamma*w2[k][j] (el 2 es por la derivada de 2 que baja el cuadrado)
    //Ver
    /*
    float gamma=nn->GAMMA;
    float weight_decay_v=0;
    for(i=1;i<=nn->N2;i++) for(j=1;j<=nn->N3;j++) weight_decay_v+= nn->w2[j][i]*nn->w2[j][i];
	  
    mse+= gamma*weight_decay_v;
    */
